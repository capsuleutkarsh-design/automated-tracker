...
class VideoPointPickerCanvas(QLabel):
    ...
    def paintEvent(self, event):
        ...
        if not self.current_pixmap:
            ...
            painter.drawText(QRectF(0, self.height()/2 - 30, self.width(), 30), Qt.AlignCenter, "📁 Drag & Drop Video or Image Sequence Here")
            painter.setFont(QFont("Segoe UI", 11))
            painter.setPen(QColor("#64748b"))
            painter.drawText(QRectF(0, self.height()/2 + 4, self.width(), 30), Qt.AlignCenter, "or select a media item from the Target Media dropdown on the left")
            return
        ...
        if not self.is_overlay_active and self.show_mask_overlay:
            for l_idx, layer in enumerate(self.layers):
                ...
                for minfo in masks_at_frame:
                    ...
                    base_color = QColor(16, 185, 129) if is_inc else QColor(244, 63, 94)
                    
                    alpha_fill = 65 if is_sel else (45 if is_active_layer else 20)
                    alpha_border = 255 if is_sel else (220 if is_kf else 140)
                    pen_style = Qt.SolidLine if (is_kf or is_sel) else Qt.DashLine
                    pen_width = 3 if is_sel else (2 if is_kf else 1)
                    ...
                    kf_tag = f"f{self.current_frame+1} 🔷" if is_kf else "Interp ~"
                    ...
                    if minfo["type"] == "poly" and minfo.get("points"):
                        ...
                        if len(poly_pts) >= 3:
                            ...
                            if is_active_layer:
                                ...
                                painter.drawText(int(top_pt.x() + 6), int(top_pt.y() - 4), label_text)
                ...
                for idx, (f_num, ox, oy) in enumerate(layer.points, start=1):
                    ...
                    if is_keyframe:
                        ...
                        painter.drawLine(int(disp_x - 12), int(disp_y), int(disp_x + 12), int(disp_y))
                        painter.drawLine(int(disp_x), int(disp_y - 12), int(disp_x), int(disp_y + 12))
                        painter.setBrush(QBrush(QColor("#f43f5e")))
                        painter.drawEllipse(QPointF(disp_x, disp_y), 4, 4)
                        painter.setPen(QColor("#ffffff"))
                        painter.setFont(QFont("Segoe UI", 9, QFont.Bold))
                        painter.drawText(int(disp_x + 8), int(disp_y - 8), f"#{idx} ({layer.name})")
                    else:
                        painter.setPen(QPen(QColor(l_qcol.red(), l_qcol.green(), l_qcol.blue(), 100), 1, Qt.DashLine))
                        painter.drawEllipse(QPointF(disp_x, disp_y), 6, 6)
                        painter.setPen(QColor(180, 180, 200, 130))
                        painter.setFont(QFont("Segoe UI", 8))
                        painter.drawText(int(disp_x + 8), int(disp_y - 4), f"#{idx} @ f{f_num+1}")
        ...
        if self.drag_start and self.drag_current:
            ...
            col = QColor("#10b981") if is_inc else QColor("#f43f5e")
        ...
        if self.current_poly:
            ...
            if self.hover_pos:
                ...
                painter.setPen(QPen(QColor(col.red(), col.green(), col.blue(), 160), 1, Qt.DotLine))
            ...
            painter.setPen(QPen(QColor(35, 45, 66), 1))
            painter.setBrush(QBrush(QColor(11, 15, 23, 230)))
            painter.drawRoundedRect(QRectF(12, self.height() - 36, 520, 26), 6, 6)
            painter.setPen(QColor("#38bdf8"))
            painter.setFont(QFont("Segoe UI", 9, QFont.Bold))
            painter.drawText(QRectF(12, self.height() - 36, 520, 26), Qt.AlignCenter, "✏️ Click to add vertices  •  Double-Click to Close  •  Right-Click to Cancel")

        # Top-Left HUD Info Pill
        alpha_tag = "   •   ⬛ ALPHA MATTE" if self.view_alpha_mode else ""
        hud_text = f"{self.orig_w} × {self.orig_h}   •   {self.fps:.1f} FPS   •   Frame {self.current_frame+1:04d} / {max(1, self.total_frames):04d}{alpha_tag}"
        hud_font = QFont("Consolas", 10, QFont.Bold)
        painter.setFont(hud_font)
        fm = painter.fontMetrics()
        pill_w = fm.horizontalAdvance(hud_text) + 32
        pill_rect = QRectF(12, 12, pill_w, 28)
        painter.setPen(QPen(QColor(56, 189, 248, 90), 1))
        painter.setBrush(QBrush(QColor(11, 15, 23, 230)))
        painter.drawRoundedRect(pill_rect, 6, 6)
        painter.setPen(QColor("#34d399" if self.view_alpha_mode else "#38bdf8"))
        ...
        if self.show_loupe and self.hover_pos and self.current_pixmap:
            ...
            loupe_radius = 52.0
            loupe_cx = cx + 75 if cx < self.width() - 150 else cx - 75
            loupe_cy = cy - 75 if cy > 150 else cy + 75
            ...
            if not patch.isNull():
                ...
                # Coordinate Badge
                coord_rect = QRectF(loupe_cx - 45, loupe_cy + loupe_radius + 4, 90, 20)
                painter.setPen(QPen(QColor(35, 45, 66), 1))
                painter.setBrush(QBrush(QColor(11, 15, 23, 230)))
                painter.drawRoundedRect(coord_rect, 4, 4)
